...
export class AudioStreamer {
  ...
  private initialBufferTime: number = 0.25; // 250ms initial buffer for jitter
  ...
  addPCM16(chunk: Uint8Array) {
    ...
    // Coalesce small chunks to reduce scheduling overhead and prevents GC pressure
    ...
    if (this.processingBuffer.length >= this.bufferSize) {
      const pcm16 = this._processPCM16Chunk(this.processingBuffer);
    ...
    if (!this.isPlaying) {
      ...
      // If we are already "playing" but the queue was empty, 
      // we might be waiting for the checkInterval. Trigger immediately instead.
      if (this.audioQueue.length > 0 && this.scheduledTime < this.context.currentTime + 0.1) {
          this.scheduleNextBuffer();
      }
    }
  }
  ...
  private scheduleNextBuffer() {
    const SCHEDULE_AHEAD_TIME = 0.4; // Schedule 400ms ahead
  ...
  // Target volume for the gain node.
  ...
  async setAudioOutput(deviceId: string) {
    try {
      if (typeof (this.context as any).setSinkId === "function") {
        // The Web Audio API uses '' for the system default; 'default' is a
        // browser-enumerated device ID that setSinkId does not accept.
